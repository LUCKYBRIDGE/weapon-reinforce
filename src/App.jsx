...
function App() {
  ...
  const [quizSessionStats, setQuizSessionStats] = useState({ correct: 0, wrong: 0, earned: 0, streak: 0 });
  ...
  const beginQuizPenalty = useCallback(({ correctAnswer, message, delayLabel, onComplete }) => {
    ...
    quizPenaltyIntervalRef.current = window.setInterval(() => {
      ...
      setQuizPenalty(prev => prev ? { ...prev, remainingSeconds } : prev);
    }, 200);
  ...
  }, [selectedQuizPackId, quizStats, reportStorageSaveFailure]);
  ...
  const prepareGoldQuizPack = async (packId, { resetRecent = false } = {}) => {
  ...
  const openGoldQuiz = () => {
    ...
    clearQuizPenalty();
    setQuizSessionStats({ correct: 0, wrong: 0, earned: 0, streak: 0 });
    setShowQuizModal(true);
    void prepareGoldQuizPack(selectedQuizPackId, { resetRecent: true });
  };

  const handleQuizPackChange = (nextPackId) => {
  ...
  const handleAnswer = (selected, e) => {
    if (quizPenalty || quizAnswerLockRef.current || quizLoadState !== 'ready' || !currentQuiz) return;
    quizAnswerLockRef.current = true;
    setQuizAnswerLocked(true);
